    ...
    const handleUpdateEstado = async (id, nuevoEstado) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/auditor/certificados/${id}/estado`, { 
                method: 'PUT', // Importante: Metodo PUT
                headers: { 
                    'Authorization': auth,              // Tu credencial
                    'Content-Type': 'application/json'  // Importante: Decirle que enviamos JSON
                },
                // El cuerpo debe coincidir con lo que espera el backend (Map<String, String>)
            ...
            if (response.ok) {
                ...
                // Actualizamos la lista en pantalla sin tener que recargar
                setCertificados(prevCertificados => 
                    prevCertificados.map(cert => 
                        cert.id === id ? updatedCert : cert
                    )
                );
                setError(''); // Limpiar errores previos si hubo
            } else {
